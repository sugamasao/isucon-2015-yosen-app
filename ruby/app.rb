...
SQL
    ...
    entries_of_friends = []
    db.query('SELECT * FROM entries ORDER BY created_at DESC LIMIT 20').each do |entry|
    ...
    comments_of_friends = []
    db.query('SELECT * FROM comments ORDER BY created_at DESC LIMIT 20').each do |comment|
